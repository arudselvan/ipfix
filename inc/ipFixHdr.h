 ...
 */
typdef struct ipfix_msg_hdr_ {
    uint16_t ver;
    uint16_t len;
    uint32_t export_time;
    uint32_t seq;
    uint32_t domain_id;   
    unint8_t sets[0];  /* Template record sets/Options Template record sets/Data sets */
} ipfix_msg_hdr_t;
...
typedef struct ipfix_set_hdr_ {
    uint16_t id;
    uint16_t len;
    unit8_t data[0]; /* Template/Options Template/
                      * Data - 
